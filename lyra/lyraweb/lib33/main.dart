...
class _SpeechScreenState extends State<SpeechScreen> {
  ...
  void _startEfendimTimeout() {
    _commandSilenceTimer?.cancel();
    _commandSilenceTimer = Timer(const Duration(seconds: 5), () async {
      if (_awaitingCommand && _isListening) {
        setState(() {
          _addMessage("Asistan: Sizi anlayamadım");
        });
        await _voiceService.speakAndWait("Sizi anlayamadım");
        await Future.delayed(const Duration(milliseconds: 500));
        setState(() {
          _isListening = true;
          _awaitingCommand = true;
        });
        await _startCommandListeningWaitTTS();
      }
    });
  }

  Future<void> _startCommandListeningWaitTTS() async {
    // TTS konuşması bitmeden dinleme başlatılmaz
  ...
  void _handleCommandResult(String recognized) async {
    if (_awaitingCommand && recognized.isNotEmpty) {
      ...
      await _voiceService.speakAndWait(yanit);
      setState(() {
        _showLyraOverlay = false;
      });
      _text = '';
    }
    // Komut gelirse timer iptal
    _commandSilenceTimer?.cancel();
  }

  Future<void> _handleSpeechResultTrigger(String recognized) async {
    ...
    if (isWakeWord) {
      ...
      await _voiceService.speakAndWait("Efendim");
      setState(() {
        _isListening = true;
        _awaitingCommand = true;
      });
      await _startCommandListeningWaitTTS();
      _startEfendimTimeout();
    } else if (recognized.isNotEmpty) {
      setState(() {
        _showLyraOverlay = false;
      });
      _addMessage("Kullanıcı: $recognized");
      await _voiceService.speakAndWait("Lütfen hey layra deyin");
      setState(() {
  ...
  Future<void> _startListening() async {
    ...
    await _voiceService.speakAndWait(""); // TTS varsa bekle, yoksa hemen devam et
    _voiceService.listen(
      onResult: (recognized) {
        final recognizedLower = recognized.toLowerCase();
        final isWakeWord = WAKE_WORD_VARIATIONS.any((w) => recognizedLower.contains(w));
  ...
  void _startCommandListening() {
    // Artık kullanılmıyor, hata vermemesi için boş bırakıldı veya kaldırılabilir.
  }
